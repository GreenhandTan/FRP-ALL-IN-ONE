...
> **📌 实例：超低配 LXC 服务器实测**  
> 通过「**本地构建 + 云端导入**」的特殊部署手法（在本地构建好 Docker 镜像后导入云服务器，绕过内存不足的构建限制），本项目可在 **1核 256MB 内存 + 2G 数据盘** 的 LXC 架构云服务器上流畅运行，实现完整的内网穿透集中管理。作者实测机器售价仅 **¥29.9 / 三年**，极具性价比。详细部署思路参见：[作者博客 Blog#25](https://greenhandtan.top/blog/25)
