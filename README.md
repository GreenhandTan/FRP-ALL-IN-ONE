...
> [!CAUTION]
> **升级提示（v3.0+）**：本次为重大架构升级（GitHub OAuth 登录 + 前端 UI 全面重构），**建议旧版本用户直接重新部署**，详见 [升级指南](#upgrade)。
...
> [!CAUTION]
> **v3.0 为重大架构升级**（登录方式从账号密码迁移至 GitHub OAuth，前端 UI 全面重构），数据库结构和前端代码均有不兼容变更。**建议直接重新部署**，无需手动执行数据库迁移。
